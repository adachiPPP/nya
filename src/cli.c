...
static void print_help(void) {
	...
	printf("  nya --save <key> <value>      save a config option (sudobin, aur, searchaur, etc.)\n");
	...
	printf("  RefreshAge = <seconds>  (default 21600 = 6h, DBs younger than this are skipped)\n");
}
...
static int do_read_paconfig(const char *dest, const char *pacpath) {
	if (is_file(dest)) {
		...
		char *cp_argv[] = { (char *)"cp", (char *)"-a", (char *)dest, bak, NULL };
		run_cmd(cp_argv);
		info("backed up %s to %s", dest, bak);
	}
...
int cli_main(int argc, char **argv) {
	...
	if (cl.op == 'P') {
		...
		else { snprintf(dest, sizeof dest, "/etc/nya.conf"); dest_is_default = 1; }
		if (geteuid() != 0 && dest_is_default) {
			const char *sudobin = "sudo";
			char sudobin_buf[64] = "sudo";
			config *tmpc = config_discover();
			if (tmpc && tmpc->sudobin && *tmpc->sudobin) {
				snprintf(sudobin_buf, sizeof sudobin_buf, "%s", tmpc->sudobin);
				sudobin = sudobin_buf;
			}
			if (tmpc) config_free(tmpc);
			...
			args[argc + 1] = NULL;
			execvp(sudobin, args);
			error("failed to elevate via '%s' (install it, or run nya as root)", sudobin);
			free(args);
			strs_free(&cl.targets);
			return 1;
		}
	...
	if (cl.save_key && cl.save_val) {
		const char *path = cl.config_path ? cl.config_path : config_default_path();
		char dir[4096];
		...
		if (config_save_kv(path, cl.save_key, cl.save_val) != 0) {
			error("failed to save %s = %s to %s", cl.save_key, cl.save_val, path);
			strs_free(&cl.targets);
			return 1;
		}
		msg("saved %s = %s in %s", cl.save_key, cl.save_val, path);
		strs_free(&cl.targets);
		return 0;
	}
	config *c;
