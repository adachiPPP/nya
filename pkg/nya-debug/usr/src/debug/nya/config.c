...
static void config_apply_key(config *c, repo *r, const char *key, const char *val) {
	if (!r) {
		...
		c->parallel = atoi(val);
	} else if (str_ieq(key, "RefreshAge")) {
		c->refreshage = atol(val);
...
static void write_options(FILE *f, config *c, int from_pac) {
	...
	if (c->refreshage > 0) fprintf(f, "RefreshAge = %ld\n", c->refreshage);
	fprintf(f, "Color\n");
	if (c->verbosepkglists) fprintf(f, "VerbosePkgLists\n");
	fprintf(f, "NyaCacheDir = %s\n", c->nyacache);
	fprintf(f, "NixChannel = %s\n", c->nixchannel);
	fprintf(f, "AurBase = %s\n", c->aurbase);
	if (c->aur) fprintf(f, "aur = true\n"); else fprintf(f, "#aur = true\n");
	if (c->nix) fprintf(f, "nix = true\n"); else fprintf(f, "#nix = true\n");
	if (c->searchaur >= 0) fprintf(f, "searchaur = %s\n", c->searchaur ? "true" : "false"); else fprintf(f, "#searchaur = true\n");
	if (c->searchnix >= 0) fprintf(f, "searchnix = %s\n", c->searchnix ? "true" : "false"); else fprintf(f, "#searchnix = true\n");
	if (c->searchflatpak >= 0) fprintf(f, "searchflatpak = %s\n", c->searchflatpak ? "true" : "false"); else fprintf(f, "#searchflatpak = true\n");
	if (c->searchhost >= 0) fprintf(f, "searchhost = %s\n", c->searchhost ? "true" : "false"); else fprintf(f, "#searchhost = true\n");
	fprintf(f, "sudobin = %s\n", c->sudobin ? c->sudobin : "sudo");
	fprintf(f, "hostsrepo = %s\n", c->hostsrepo ? c->hostsrepo : "https://adachippp.github.io/nya-hosts");
	if (c->aurfirst) fprintf(f, "aurfirst = true\n"); else fprintf(f, "#aurfirst = true\n");
	(void)from_pac;
}
...
int config_save_kv(const char *path, const char *key, const char *val) {
	config *c = config_alloc();		if (is_file(path)) {
			if (parse_config_file(c, path, 0, NULL) != 0) {
				config_free(c);
				return -1;
			}
		} else {
			config *pac = config_alloc();			if (config_read_pacman(pac, "/etc/pacman.conf") == 0) {
				free(c->rootdir); c->rootdir = xstrdup(pac->rootdir);
			free(c->dbpath); c->dbpath = xstrdup(pac->dbpath);
			free(c->arch); c->arch = xstrdup(pac->arch);
			strs_free(&c->cachedirs); c->cachedirs = pac->cachedirs;
			memset(&pac->cachedirs, 0, sizeof pac->cachedirs);
			strs_free(&c->siglevel); c->siglevel = pac->siglevel;
			memset(&pac->siglevel, 0, sizeof pac->siglevel);
			c->parallel = pac->parallel;
			c->color = pac->color;
			c->verbosepkglists = pac->verbosepkglists;
			int i;
			for (i = 0; i < c->nrepos; i++) repo_free(c->repos[i]);
			free(c->repos);
			c->repos = pac->repos; c->nrepos = pac->nrepos;
			pac->repos = NULL; pac->nrepos = 0;
		}
		config_free(pac);
	}
	config_apply_key(c, NULL, key, val);
	config_resolve_arch(c);
	int rc = config_write(path, c, 0);
	config_free(c);
	return rc;
}
...
const char *config_default_path(void) {
	...
	if (geteuid() == 0) return "/etc/nya.conf";
	static char buf[4096];
	const char *home = getenv("HOME");
	snprintf(buf, sizeof buf, "%s/.config/nya/nya.conf", home ? home : ".");
	return buf;
}
